...
create or replace function uni_fun() returns
  ...
  declare c integer;
  begin
    select count(*) into c from evento_reposicao  where new.unidades > max_unidades(select unidades from planograma where ean = new.ean));
    if c > 0 then
...
create or replace function pro_fun() returns
  ...
  declare c integer;
  begin
    select count(*) into c from evento_reposicao  where new.ean = 
              prodean(select ean from prateleira where nome = prodcat(select nome from tem_categoria where new.ean = ean));
    if c = 0 then
...
create trigger pro_trg before update on evento_reposicao for each row execute procedure pro_fun();

create trigger pro_trg before insert on evento_reposicao for each row execute procedure pro_fun();